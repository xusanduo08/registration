...
export function useFormItem(params) {
  const { label, required = false, validator, formItemProps = {}, beforeUploadValidator, onSuccess } = params || {};
  ...
  function onFileUpload(e) {
    ...
    if (beforeUploadValidator && typeof beforeUploadValidator === 'function' && beforeUploadValidator(file, errorInfo => {setInfo(errorInfo);setHasError(true)})) {
      // 开始上传
      ...
      request('/upload', { method: 'post', data: formData, requestType: 'form',}).then(resp => {
        ...
        if(resp.status === 'success') {
          ...
          setValue({ file: { name: get(file, 'name'), type: get(file, 'type')}, url: resp.data });
  ...
  const formItemType = {
    INPUT: <input onChange={e => onValueChange(get(e, 'target.value'))} {...formItemProps} />,
    DATEPICKER: <AntdDatePicker onChange={onValueChange} {...formItemProps} />,
    UPLOAD: <><label className={styles.upload} disabled={uploadDisabled}>
      <input type='file' style={{ display: 'none' }} {...formItemProps} onChange={onFileUpload} disabled={uploadDisabled} />
    ...
    {previewSrc && (
      <Spin spinning={uploadLoading}>
        <label className={styles.upload}>
          <img className={styles.delete} src={deleteLogo} onClick={deleteFile}/>
          <img className={styles.preview} src={previewSrc} height='100%' width='100%' />
        </label>
      </Spin>
  ...
  const formItem = useMemo(() => formItemType[params.type], [params.type, previewSrc]);
  return [(
